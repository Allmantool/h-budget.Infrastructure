...
import { AddRange, FetchAllCurrencyRates } from 'app/modules/shared/store/actions/currency-rates.actions';
...
describe('Currency rates store', () => {
    ...
    const initialStoreRateGroups: CurrencyRateGroup[] = new Array<CurrencyRateGroup>(
        {
            currencyId: 1,
            currencyRates: [
                {
    ...
    beforeEach((() => {
        ...
        TestBed.configureTestingModule({
            ...
            providers: [
                { provide: NationalBankCurrencyProvider, useValue: currencyRateProviderSpy }
            ]
        }).compileComponents();
    ...
    it('it "AddRange": initial setup - expect 2 carrency groups', (() => {
        store.selectOnce(state => state.currencyRateState.rateGroups).subscribe(groups => {
    ...
    it('it "FetchAllCurrencyRates": update existed currency groups - expect predicte amount of rates within groups', (() => {
        ...
        const groups = store.selectSnapshot(state => state.currencyRateState.rateGroups);
        const items = _.flattenDeep(_.map(groups, g => g.currencyRates));
    ...
    it('it "GetCurrencyRatesFromPreviousDay": return expected previous date currency rates', (() => {
        ...
        const previousDayRates = CurrencyRatesState.getCurrencyRatesFromPreviousDay(stubValue);
    ...
    it('it "GetCurrencyRatesByCurrencyId": return a currency group by id', (() => {
        const carrencyGroup = CurrencyRatesState
            .getCurrencyRatesByCurrencyId(store.selectSnapshot(state => state.currencyRateState))(2);
